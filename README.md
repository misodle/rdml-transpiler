# RDML Transpiler

Transpiler for the RDML programing language. 

# Target Languages

The initial target language is PHP.
Possible future language targets include C# (OO) and Elixir (functional). 

# Implementation

This project is a based upon the work of Viewpoints Research Institute [VPRI] (https://vpri.org/). 
Specifically the OMeta JS libary [Ometa] (https://b-studios.de/ometa-js/). 
Ometa is a PEG parser which unifies and simplifies the typical compiler tool change.


## Why This Exists

The goal of this project is to provide the ability to convert legacy 4GL RDML code into a more modern language while preserving symantic behavior and compatability where possible. 

## Road Map Items (High Level)
- [x] Basic RDML commands (cleanup - coming soon)
- [x] PHP output backend
- [ ] Samples (future)
- [ ] RDML extended commands (future)
- [ ] BIFS - built in functions (future)
- [ ] C# backend (future)
- [ ] Elxir backend (future)

## License

MIT License - Free to use, modify, and distribute.

[Demo] [Documentation] [Contributing]