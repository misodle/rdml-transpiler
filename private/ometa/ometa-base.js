...
OMeta = {
  ...
  listOf: function(rule, delim) {
    return this._or(function() {
                      ...
                      return this._many(function() {
                                          this._applyWithArgs("tokenOrig", delim)
  ...
  token: function(cs) {
    this._apply("spaces")
    return this._applyWithArgs("seqNoCase", cs)
  },
  tokenOrig: function(cs) {
    this._apply("spaces")
    return this._applyWithArgs("seq", cs)
  },
  seqNoCase: function(xs) {
    for (var idx = 0; idx < xs.length; idx++)
      this._applyWithArgs("exactlyNoCase", xs.at(idx))
    return xs
  },
  exactlyNoCase: function(wanted) {
    if (wanted.toUpperCase() === this._apply("anything").toUpperCase())
      return wanted
    throw fail
  },
  fromTo: function (x, y) {
