...
ometa CalcParser { 
  	start				= "WHERE(" whereBody:w  ")"			    -> w
						| anything+:xs   						-> { debug("dbg start2nd: " + xs.join('')); return 'N/A' },
...
// Load Grammer from file instead of from variable
var testGrammar2 = read("test_grammer2.txt");
...
//test("start", `WHERE( (#player_id = 1654) )`);
//test("start", `
//WHERE( (#player_id = 1654) )
//`);
//test("start", `
//  WHERE( (#player_id = 1654) )
//`);
//test("start", `
//SELECT  WHERE( (#player_id = 1654) )
//`);
//test("start", "SELECT WHERE( (#player_id = 1654) )");
//test("start", "WHERE( (#player_id = 1654) )");
test("start", "WHERE((#player_id = 1659))");
